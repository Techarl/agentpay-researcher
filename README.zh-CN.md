...
[English README](./README.md) | [中文威胁模型](./docs/THREAT_MODEL.zh-CN.md) | [真实支付说明](./docs/REAL_PAYMENTS.zh-CN.md)

一个最小化的 TypeScript AI 研究代理示例，用 HTTP `402 Payment Required` 风格的流程演示机器到机器支付如何解锁付费搜索 API。

主项目只使用模拟支付。它不会读取真实私钥，不会连接真实钱包，也不会转移真实资金。真实支付实验隔离在 `examples/` 目录，并且默认是 dry-run 或 testnet 方向。
...
- [`examples/x402-real-buyer`](./examples/x402-real-buyer)：隔离的真实 x402 buyer CLI。默认 dry-run；testnet 需要 funded burner wallet；mainnet 必须显式传入 `--mainnet --confirm-real-money`。
...
真实支付安全规则见 [docs/REAL_PAYMENTS.zh-CN.md](./docs/REAL_PAYMENTS.zh-CN.md)。本项目不自动 bridge，也不自动 swap。Dry-run 不花钱；testnet 可能需要 faucet token；mainnet 使用真实资金，必须显式确认。
...
| `X402_PRIVATE_KEY` | 隔离 x402 real buyer 示例使用的私钥，只能放 `.env`，不能提交 |
| `X402_NETWORK` | x402 buyer 网络，默认 `base-sepolia` 别名 |
| `X402_RPC_URL` | 可选 RPC URL |
| `X402_ALLOWED_ENDPOINTS` | 真实 x402 buyer endpoint allowlist |
| `X402_DEFAULT_ENDPOINT` | 可选默认 x402 buyer endpoint |
| `X402_MAX_PER_REQUEST_USD` | 单次真实支付预算上限 |
| `X402_MAX_PER_RUN_USD` | 单次运行真实支付预算上限 |
| `X402_ALLOWED_NETWORKS` | network allowlist |
